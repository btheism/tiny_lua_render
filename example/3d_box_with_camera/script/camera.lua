...
function camera:process_input()
    ...
    if window.key_pressed("w") and self.pitch-self.sensitive<math.rad(89.0)
    then
        self.pitch=self.pitch+self.sensitive
    end
    if window.key_pressed("s") and self.pitch-self.sensitive>math.rad(-89.0)
    then
        self.pitch=self.pitch-self.sensitive
    end
