...
simple_fragment_shader_code = [[
    ...
    void main()
    {
        FragColor = mix(texture(Tex0, TexPos), texture(Tex1, TexPos), mix_factor);
    }
]]
...
square.tex[2] = texture.new_texture_2d("image/keli.jpg", "rgb", "rgba", "repeat", "repeat", "linear", "linear")
...
function square.draw(acc_time)
    for i=1,#square.tex do
        square.tex[i]:active(i-1)
    end
    square.shader:use()
    square.shader:set_float("mix_factor", (math.sin(180*acc_time)+1.0)/2.0)
    square.mesh:draw()
end
